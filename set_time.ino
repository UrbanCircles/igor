...
String menuOptions[4] = {"UP", "DOWN", "Reset", "SET"}; // Label reset option as "Reset"
...
void updateDisplay()
{
    ...
    {
        mainRowText = String(manualValue); // Display manual value being set
    }
...
void handleButtonPresses(unsigned long currentMillis)
{
    ...
    case SET_MINUTES:
        flowMinutes = manualValue; // Save the manually set value
        currentState = MENU;       // Return to menu
        Serial.println("Flow minutes manually set to: " + String(flowMinutes));
        break;
    }
    updateDisplay();
}
...
void setFlowMinutes()
{
    ...
    manualValue = flowMinutes;
    Serial.println("Flow minutes Updated Manually.");
    updateDisplay();
}
...
void handleRotaryInput()
{
    ...
    if (currentState == MENU)
    {
        menuIndex = (menuIndex + rotation + 4) % 4; // Update for 3 menu options: UP, DOWN, Reset
        updateDisplay();
        Serial.print(millis()); // Print the current time in milliseconds
    ...
    {
        manualValue = max(0, manualValue + rotation);
        updateDisplay();
        Serial.print(millis()); // Print the current time in milliseconds
